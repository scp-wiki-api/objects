...
<details>
<summary>+ Excerpt from Document 2223-56-Ackermann, Section II: Detection of SCP-2223</summary>

Protocol 56-Ackermann requires a modification to the PageRank algorithm used by internet search engines. Under the assumption of a random-surfer model with a damping factor  and  pages or images, one can consider a PageRank metric , a critical factor in ranking search results:

PR(p_i) = \frac{1-d}{N} + d \sum \frac{PR(p_j)}{L(p_j)}

Summed over pages with inbound links to  where  represents a page and  outputs the number of outbound links from . One can represent this system of linear equations as the following linear transformation: 
...
Where  is 0 if  does not link to  and otherwise some non-zero value normalized such that the columns of the matrix  are stochastic.  is a matrix filled with 1's and is guaranteed to return  due to the stochastic nature of vector . The unique solution vector  represents the eigenvector associated with the eigenvalue 1 for matrix  and yields the PageRank values used in search result ranking. The eigenvalue 1 is the largest such eigenvalue and thus efficient methods for retrieving dominant eigenvectors, like power iteration, may be used (see *Section III: Computational Methods* for further details). 
...
The matrix  is guaranteed a spectral gap greater than or equal to  for damping factor , and the spectral gap is equal to  if  models a graph with two or more irreducible closed subsets. Protocol 56-Ackermann requires the calculation of the second-most dominant eigenvector, an operation that can be efficiently achieved using algorithms like the block power method or Tarjan's algorithm (see *Section III: Computational Methods* for further details). 

For each entry corresponding to  in the second eigenvector ,  is an element of an irreducible closed subset if  is non-zero. In accordance with Protocol 56-Ackermann, images with this property are removed and thus SCP-2223 instances are filtered from image search resultsAckermann, D., & Vogel, S. (2011). Possible method for machine identification of SCP-2223 instances. *Terminal: An SCP Foundation Journal*, 15(72), 12-56. Retrieved from *SCP Foundation Research Database*. (Accession No. 32542432) .

</details>

Protocol 56-Ackermann modifies the PageRank algorithm used in search engines affected by SCP-2223 to rank relevance of search results. PageRank formulates a system of linear equations modeling a network of connected pages and images, which can then be expressed in the general form  for matrix A and vectors x and b. Protocol 56-Ackermann finds the eigenvectorAn eigenvector is a vector that returns a multiple of itself after being multiplied with a linear transformation matrix, while an eigenvalue is the multiplying factor. for the second-largest eigenvalue of the matrix A (PageRank retrieves page importance data from the dominant eigenvector associated with the largest eigenvalue). Using data from the second-most dominant eigenvector, Protocol 56-Ackermann determines whether an image satisfies the property of being in an irreducible closed subset indicative of a possible SCP-2223 version. If the image satisfies this property, it is excluded from search results and thus new versions of SCP-2223 are suppressed.
...
<< [SCP-2222](SCP-2222) | SCP-2223 | [SCP-2224](SCP-2224) >>

=====

> **Filename:** SCP-2223-small.png

> **Author:** PandoraNuker

> **License:** CC BY-SA 3.0

> **Source Link:** [SCP Wiki](https://scp-wiki.wikidot.com/scp-2223)
...
> **Source Link:** [Link](https://www.flickr.com/photos/107413454@N04/16476895346/)
