...
Explorers who follow the melody will be able to trace it to an exact replica of the Siena Cathedral (with previously mentioned environmental changes). The cathedral is entirely empty except for a single pipe organ which has had its longest pipes cut in half and the corresponding pedalboards torn out.The longest pipes play the lowest notes in an organ, which would leave it only able to reproduce high-pitched notes.
